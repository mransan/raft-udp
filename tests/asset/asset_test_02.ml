...
let main () =
  ...
  >>=(fun (app, asset_id, prev_tx_id) ->
    let transfer, tx_id = Asset_utl.make_transfer 
      ~prev_tx_id
      ~asset_id
  ...
  >>=(fun (app, asset_id, prev_tx_id) -> 
    let accept_transfer, tx_id = Asset_utl.make_accept_transfer 
      ~prev_tx_id
      ~asset_id
      ~prv_key:receiver
      ()
